...
def main():
    ...
    for key in input_pars.keys():
        ...
        if ('parameters' in key) or ('modes' in key) or ('modes-w-deviation' in key) or ('ordering' in key) or ('bounds' in key) or ('compare-ordering' in key ) or ('truths' in key) or ('qnms-values' in key) or ('IMR-fits' in key) or ('IMR-fits-IMR' in key) or ('Amp-Phase-fits' in key) or ('peaktime-22-all' in key) or ('time-shift' in key):
